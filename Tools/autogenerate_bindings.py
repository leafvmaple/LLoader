...
def my_litgen_options() -> litgen.LitgenOptions:
    ...
    options.source_encoding = 'gbk'  # 或其他适合的编码
...
def autogenerate() -> None:
    ...
    include_dir = os.path.join(repository_dir, "Include")
    header_files = [os.path.join(include_dir, "IAnimation.h")]

    output_cpp_pydef_file = os.path.join(repository_dir, "Src", "bindings.cpp")

    litgen.write_generated_code_for_files(
        options=my_litgen_options(),
        input_cpp_header_files=header_files,
        output_cpp_pydef_file=output_cpp_pydef_file,
        output_stub_pyi_file=repository_dir + "/__init__.pyi",
    )
